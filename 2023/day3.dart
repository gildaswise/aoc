...
].map((e) => e.replaceAll("\\", "").trim()).toList();

const List<String> testInput1 = [
  "467..114..",
  "...*......",
  "..35..633.",
  "......#...",
...
final List<String> input = realInput;
...
void main() {
  final Set<FoundNumber> allNumbers = {};
  final Set<FoundSymbol> allSymbols = {};

  // Get all numbers first
  for (int y = 0; y < input.length; y++) {
    final line = input[y].replaceAll("\\", "").trim();
    final numbersInLine = <FoundNumber>[];
    final symbolsInLine = <FoundSymbol>[];

    final numbers = numberRegex.allMatches(line);
    for (RegExpMatch match in numbers) {
      final value = match.group(0);
      if (value != null) {
        final number = (int.parse(value), match.start, y, match.end - 1);
  ...
  SymbolResult getSumForSymbol(int x, int y) {
    int sum = 0;
    List<FoundNumber> numbersUsed = [];
    for (FoundNumber number in allNumbers) {
      final numberX = number.$2;
      final numberY = number.$3;
      final numberL = number.$4;

      bool isReachingNumber(int value) {
        if (value < 0 || value >= 140) return false;
        return value >= numberX && value <= numberL;
      }

      bool shouldSum = false;
      if (numberY == y - 1 && isReachingNumber(x - 1)) {
        // top left
        shouldSum = true;
      } else if (numberY == y - 1 && isReachingNumber(x)) {
        // above
        shouldSum = true;
      } else if (numberY == y - 1 && isReachingNumber(x + 1)) {
        // top right
        shouldSum = true;
      } else if (numberY == y && isReachingNumber(x - 1)) {
        // left
        shouldSum = true;
      } else if (numberY == y && isReachingNumber(x + 1)) {
        // right
  ...
  for (FoundSymbol symbol in allSymbols) {
    ...
    if (symbol.$1 == "*") {
      ...
      if (result.$2 > 0) gearProduct += result.$2;
    } else {
      result = getSumForSymbol(symbol.$2, symbol.$3);
    }
