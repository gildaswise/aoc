...
final numberRegex = RegExp(r'(([^,]+) (red|green|blue))');
...
void main() {
  ...
  for (String game in input) {
    // Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
    final parts = game.replaceAll('Game ', '').split(": ");
    final id = int.parse(parts[0]);
    final rounds = parts[1].split(";");
    final colorCounts = {
      "red": 0,
      "green": 0,
      "blue": 0,
    ...
    for (String round in rounds) {
      final matches = numberRegex.allMatches(round);
      for (RegExpMatch match in matches) {
        final colorCount = match.group(2);
        final colorName = match.group(3);
        if (colorCount != null && colorName != null) {
          final currentCount = colorCounts[colorName]!;
          final newCount = int.parse(colorCount);
          if (newCount >= currentCount) {
            colorCounts[colorName] = newCount;
          }
        }
      }
    }
    print("Max counts for game $id: $colorCounts");
    // solution for part 1:
    bool isValid = true;
    if (colorCounts['red']! > maxColorCounts['red']!) {
      isValid = false;
    } else if (colorCounts['green']! > maxColorCounts['green']!) {
      isValid = false;
    } else if (colorCounts['blue']! > maxColorCounts['blue']!) {
      isValid = false;
    }
    if (isValid) count += id;
    // solution for part 2:
    final gamePower = colorCounts.values.reduce((t, v) => t * v);
    print("Power for game $id: $gamePower");
